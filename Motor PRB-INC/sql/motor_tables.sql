...
-- ----------------------------------------------------------------------------
-- ALTER condicional: 7 colunas novas em motor_validacao_entrega (Radar CT V2)
-- ----------------------------------------------------------------------------
-- Volumetria pré-resolução + delta de chamados pré/pós (KPI do Change Team).
-- Pra Postgres 9.2/9.3 (sem ADD COLUMN IF NOT EXISTS) usamos DO block.
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_schema='lwsa' AND table_name='motor_validacao_entrega'
                     AND column_name='grupo_designado') THEN
        ALTER TABLE lwsa.motor_validacao_entrega
            ADD COLUMN grupo_designado varchar(255);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_schema='lwsa' AND table_name='motor_validacao_entrega'
                     AND column_name='data_abertura_prb') THEN
        ALTER TABLE lwsa.motor_validacao_entrega
            ADD COLUMN data_abertura_prb timestamp with time zone;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_schema='lwsa' AND table_name='motor_validacao_entrega'
                     AND column_name='qtd_incs_pre_resolucao') THEN
        ALTER TABLE lwsa.motor_validacao_entrega
            ADD COLUMN qtd_incs_pre_resolucao int NOT NULL DEFAULT 0;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_schema='lwsa' AND table_name='motor_validacao_entrega'
                     AND column_name='clientes_unicos_pre') THEN
        ALTER TABLE lwsa.motor_validacao_entrega
            ADD COLUMN clientes_unicos_pre int NOT NULL DEFAULT 0;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_schema='lwsa' AND table_name='motor_validacao_entrega'
                     AND column_name='categorias_pre') THEN
        ALTER TABLE lwsa.motor_validacao_entrega
            ADD COLUMN categorias_pre int NOT NULL DEFAULT 0;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_schema='lwsa' AND table_name='motor_validacao_entrega'
                     AND column_name='palavra_chave_chamados') THEN
        ALTER TABLE lwsa.motor_validacao_entrega
            ADD COLUMN palavra_chave_chamados varchar(100);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_schema='lwsa' AND table_name='motor_validacao_entrega'
                     AND column_name='chamados_pre') THEN
        ALTER TABLE lwsa.motor_validacao_entrega
            ADD COLUMN chamados_pre int NOT NULL DEFAULT 0;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_schema='lwsa' AND table_name='motor_validacao_entrega'
                     AND column_name='chamados_pos') THEN
        ALTER TABLE lwsa.motor_validacao_entrega
            ADD COLUMN chamados_pos int NOT NULL DEFAULT 0;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_schema='lwsa' AND table_name='motor_validacao_entrega'
                     AND column_name='delta_chamados_pct') THEN
        ALTER TABLE lwsa.motor_validacao_entrega
            ADD COLUMN delta_chamados_pct numeric(6,3) NOT NULL DEFAULT 0;
    END IF;
END $$;
