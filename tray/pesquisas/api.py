...
# Adiciona o diretório tray ao Python Path (módulo notifica)
...
schema = 'tray'  
rotina = "Tray (Carrega Pesquisas Chat)"

config_file_path = Path(__file__).resolve().parent.parent.parent / 'config.ini'
config = configparser.ConfigParser()
config.read(config_file_path, encoding='utf-8')
DDL_URL = config['tray']['DDL_URL'].strip()
TOKEN = config['tray']['TOKEN'].strip()
VERIFY_SSL = (
    config.getboolean('tray', 'verify_ssl')
    if config.has_option('tray', 'verify_ssl')
    else True
)
if not VERIFY_SSL:
...
def get_pesquisas(DDL_URL,TOKEN,page):
    ...
    endpoint = (
        f"{DDL_URL}/survey/submissions?page={page}&limit={limit}"
        f"&type=chat&isAnswered=true&createdBetween={data_ontem}&createdBetween={data_amanha}"
    )
    ...
    response = requests.get(
        endpoint, headers=headers, timeout=60, verify=VERIFY_SSL
    )
...
try:    
    ...
    if df_content.empty:
        ...
        notify_slack_success(f"{linhas_inseridas} linhas inseridas na tabela {tabela}.:tray:",linhas_inseridas,linhas_atualizadas, rotina)
    else:
