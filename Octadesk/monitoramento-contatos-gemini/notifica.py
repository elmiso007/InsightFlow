...
#destinatarios = ['C092EF6JAR1'] #Canal Oficial Monitoramento Octa
...
def notifica(content,percentual,media):
	...
	"blocks": [
            {
                ...
                "text": {
                    "type": "plain_text",
                    "text": ":-alert:  Monitoramento de contatos Recebidos  :octadesk:",
                }
            },
            {
                "type": "section",
                "text": {
                    "type": "mrkdwn",
                    "text": f"O sistema de monitoramento de contatos identificou um aumento de *`{percentual}%`* na quantidade de contatos recebidos em relação a média de *`{media}`* contatos da última semana para esta janela de horário."
                }
            },
            {
                "type": "section",
    ...
    for destinatario in destinatarios:
        ...
        if destinatario.startswith('U'):
            ...
            response_message = client.chat_postMessage(channel=channel_id, blocks=mensagem["blocks"], text= f"Monitoramento de Contatos Octa")
        ...
        # Verificar se o destinatário começa com 'C'
            ...
            response_message = client.chat_postMessage(channel=destinatario, blocks=mensagem["blocks"], text=f"Monitoramento de Contatos Octa")
...
def notifica_boas_noticias(hora_inicio, hora_fim,percentual):
    ...
    for destinatario in destinatarios:
        ...
        if destinatario.startswith('U'):
            ...
            # Enviar a mensagem no direct
            response_message = client.chat_postMessage(channel=channel_id, blocks=mensagem["blocks"], text= f"Monitoramento de Contatos Octadesk")
            print(f"Mensagem enviada para o direct do usuário {destinatario}.")
        # Verificar se o destinatário começa com 'C'
        elif destinatario.startswith('C'):
            # Usar o código que já possui para destinatários que começam com 'C'
            response_message = client.chat_postMessage(channel=destinatario, blocks=mensagem["blocks"], text=f"Monitoramento de Contatos Octadesk")
            print(f"Mensagem enviada para {destinatario}.")
