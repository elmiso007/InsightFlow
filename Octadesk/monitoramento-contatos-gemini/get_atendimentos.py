...
def get_atendimentos(data_inicio, data_fim):
    ...
    SELECT
        ...
        c.data_fim_interacao,
        c.contact_name,
        c.agent_name AS analista,
        c.grupo_nome AS fila,
        m.mensagens,
        dg.equipe,
        dg.setor,
        c.canal        
    FROM octadesk.chat c 
    LEFT JOIN octadesk.mensagens m ON c.id = m.id
    LEFT JOIN octadesk.depara_grupo dg ON c.grupo_nome = dg.grupo
       WHERE c.data_inicio_interacao BETWEEN '{data_inicio}' AND '{data_fim}' AND dg.setor in ('Suporte');
    '''
    ...
    for _, row in df_mensagem.iterrows():  # Itera sobre as linhas do DataFrame
        ...
        try:
            ...
            if isinstance(item, str) and item.strip():
                ...
                # Loga itens vazios ou não JSON, mostrando o conteúdo para análise
                print(f"Mensagem vazia ou formato inválido. Conteúdo: {repr(item)}")
