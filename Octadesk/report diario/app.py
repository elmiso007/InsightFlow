...
config_folder = r'C:\Users\lucas.abner\Desktop\Rotinas Python\Octadesk\report diario'
...
def soma_points(df, canal=None,setor= None, ultima_semana=None, data=None):
    ...
    soma_fourth = '-'  # valor padrão para quarto ponto
    ...
    # Verificar os intervalos de tempo e realizar a soma do quarto ponto
    if hora_atual >= datetime.strptime("23:00:00", "%H:%M:%S").time():
        soma_fourth = int(df['fourth_point'].sum())  # apenas soma se a condição for verdadeira

    return soma_first, soma_second, soma_third, soma_fourth
...
def padrao(df, canal=None,setor= None, ultima_semana=None, data=None,diautil=None):
    ...
    if diautil:
     ...
     # Filtrar dias úteis dentro do período de padrão definido
    total = df[(df['diautil'] == 1) & (df['Data Inicio'] >= padrao_inicio) & (df['Data Inicio'] <= padrao_fim)]  # Filtra dias úteis no período
    total = total['data'].nunique()  # Conta o número de valores únicos na coluna 'data'
    print(f"Total de dias úteis no período ({padrao_inicio.date()} a {padrao_fim.date()}): {total}")
    ...
    soma_fourth_padrao = '-'  # valor padrão para quarto ponto
    ...
    # Verificar os intervalos de tempo e realizar a soma do quarto ponto
    if hora_atual >= datetime.strptime("23:00:00", "%H:%M:%S").time():
        total_fourth_padrao = df['fourth_point'].sum()  # apenas soma se a condição for verdadeira
        soma_fourth_padrao = round(total_fourth_padrao/total)

    return soma_first_padrao, soma_second_padrao, soma_third_padrao, soma_fourth_padrao
...
padrao_inicio = pd.to_datetime('2025-08-01 00:00:00')  # ou a data que você estiver utilizando
padrao_fim = pd.to_datetime('2025-10-31 23:59:59')

hoje_inicio, hoje_fim = obter_hoje()

# Consulta SQL
query = f'''SELECT * FROM octadesk.vw_report_diario_filtrada; '''
...
for destinatario in destinatarios:
        ...
        if destinatario.startswith('U'):
            ...
            response_message = client.chat_postMessage(channel=channel_id, blocks=mensagem["blocks"], text= f"Report Diário Octadesk {data_formatada}")
        ...
        # Verificar se o destinatário começa com 'C'
            ...
            response_message = client.chat_postMessage(channel=destinatario, blocks=mensagem["blocks"], text=f"Report Diário Octadesk {data_formatada}")
...
for setor in setores:
    ...
    #Soma padrão
    soma_first_padrao, soma_second_padrao, soma_third_padrao, soma_fourth_padrao = padrao(df,setor=setor,data=True,diautil=True)

    # Soma semana anterior
    soma_first_semana_anterior_wp, soma_second_semana_anterior_wp, soma_third_semana_anterior_wp, soma_fourth_semana_anterior_wp = soma_points(df, canal='WhatsApp', setor=setor, ultima_semana=True)
    soma_first_semana_anterior_chat,soma_second_semana_anterior_chat,soma_third_semana_anterior_chat, soma_fourth_semana_anterior_chat = soma_points(df, canal='Chat',setor=setor,ultima_semana=True)
    soma_first_semana_anterior, soma_second_semana_anterior, soma_third_semana_anterior, soma_fourth_semana_anterior = soma_points(df,canal=None,setor=setor, ultima_semana=True)

    #Soma dia atual
    soma_first_wp,soma_second_wp,soma_third_wp, soma_fourth_wp = soma_points(df, canal='WhatsApp',setor=setor,ultima_semana=0,data=True)    
    soma_first_chat,soma_second_chat,soma_third_chat, soma_fourth_chat = soma_points(df, canal='Chat',setor=setor,ultima_semana=0,data=True)
    soma_first, soma_second, soma_third, soma_fourth = soma_points(df,canal=None,setor=setor, ultima_semana=0,data=True)
    ...
    percentual_padrao4 = percentual_padrao(soma_fourth,soma_fourth_padrao) * 100
    ...
    percentual_semana_anterior4 = percentual_padrao(soma_fourth,soma_fourth_semana_anterior) * 100
...
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    tabela = {
        "": ["Padrão Média dia Útil - Ago. a Out. 2025",
        ...
        "00:00 as 23:59": 
            [soma_fourth_padrao,
            soma_fourth_semana_anterior_wp,
            soma_fourth_semana_anterior_chat,
            soma_fourth_semana_anterior,
            soma_fourth_wp,
            soma_fourth_chat,
            soma_fourth,
            0,
    ...
    # Configurar a figura para um tamanho maior para acomodar a quarta coluna
    ...
    # Definir as larguras das colunas para 5 colunas
    col_widths = [0.25, 0.08, 0.08, 0.08, 0.08]
    ...
    table[0, 4].set_facecolor('lightblue')
    ...
    table[2, 4].set_facecolor('#F3A07D')
    ...
    table[3, 4].set_facecolor('#F3A07D')
    ...
    table[4, 4].set_facecolor('#F3A07D')
    ...
    table[5, 4].set_facecolor('#9AC09A')
    ...
    table[6, 4].set_facecolor('#9AC09A')
    ...
    table[7, 4].set_facecolor('#9AC09A')
    ...
    ajustar_celula(table[8, 4], percentual_padrao4)
    ...
    ajustar_celula(table[9, 4], percentual_semana_anterior4)
