...
task = 'monitoramento_contatos_octadesk'
...
def verificar_horario_operacional(hora_atual):
    ...
    hora_fim_execucao = datetime.strptime("20:00:00", "%H:%M:%S").time()

    if not (hora_inicio_execucao <= hora_atual <= hora_fim_execucao):
        print(f"Horário atual {hora_atual} está fora do intervalo permitido (06:00 às 20:00). Encerrando.")
        sys.exit()
...
def limpar_formatacao_slack(texto):
    """
    Limpa e corrige a formatação do texto para exibição correta no Slack.
    """
    # 1. Substitui ** por * (markdown padrão para Slack)
    texto = re.sub(r'\*\*', '*', texto)
    
    # 2. Remove asteriscos usados como marcadores de lista (ex: "*   item" ou "* item")
    texto = re.sub(r'^\s*\*\s{1,}', '• ', texto, flags=re.MULTILINE)
    
    # 3. Remove asteriscos órfãos no início de linhas (sem par de fechamento na mesma linha)
    linhas = texto.split('\n')
    linhas_limpas = []
    for linha in linhas:
        # Conta asteriscos na linha
        asteriscos = linha.count('*')
        # Se número ímpar de asteriscos, há um órfão - remove asteriscos soltos
        if asteriscos % 2 != 0:
            # Remove asterisco no início da linha se não tiver par
            if linha.strip().startswith('*') and not re.match(r'^\s*\*[^\s*]+.*\*', linha):
                linha = re.sub(r'^\s*\*\s*', '', linha)
            # Remove asterisco no final da linha se não tiver par
            elif linha.strip().endswith('*') and linha.strip().count('*') == 1:
                linha = re.sub(r'\s*\*\s*$', '', linha)
        linhas_limpas.append(linha)
    
    texto = '\n'.join(linhas_limpas)
    
    # 4. Remove linhas que contêm apenas asteriscos ou espaços
    texto = re.sub(r'^\s*\*+\s*$', '', texto, flags=re.MULTILINE)
    
    # 5. Remove múltiplas linhas em branco consecutivas
    texto = re.sub(r'\n{3,}', '\n\n', texto)
...
#QUERY E CONEXÃO COM O BANCO E CONDIÇÃO DE VIRIFICAÇÃO SOMENTE EM DIA ÚTIL
...
if df.empty:
    ...
    if percentual > -150 and atendimentos_atuais > 3:
        ...
        # Aplicando limpeza de formatação para Slack
        content = limpar_formatacao_slack(content)
