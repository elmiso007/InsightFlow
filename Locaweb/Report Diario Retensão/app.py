...
print(f"Conectando ao banco: {server}/{database} com usuário: {uid}")
...
if data_hoje.weekday() < 5 and data_hoje not in feriados_brasil:
    ...
    print(f"{data_hoje} não é um dia útil. A aplicação será encerrada.")
    exit()
...
def calcular_dias_uteis_periodo(data_inicio, data_fim):
    """
    Calcula o total de dias úteis entre duas datas (inclusive)
    Considera apenas segunda a sexta-feira, excluindo feriados brasileiros
    """
    ...
    while data_atual <= data_final:
        # Verifica se é dia útil (segunda a sexta, 0-4)
        if data_atual.weekday() < 5:
            # Verifica se não é feriado
            if data_atual not in feriados_brasil:
                dias_uteis += 1
...
def padrao(df, canal=None,setor= None, ultima_semana=None, data=None,diautil=None):
    ...
    # Calcular total de dias úteis do período padrão
    total = calcular_dias_uteis_periodo(padrao_inicio, padrao_fim)
    print(f'Total de dias úteis no período padrão ({padrao_inicio.date()} a {padrao_fim.date()}): {total}')


    # Inicializar a soma
    total_first_padrao = df['first_point'].sum()
    ...
    print(f'Padrão 10hs: {soma_first_padrao}')
    ...
    if hora_atual >= datetime.strptime("14:00:00", "%H:%M:%S").time():
        ...
        print(f'Padrão 14hs: {soma_second_padrao}')
    # Verificar os intervalos de tempo e realizar a soma
    if hora_atual >= datetime.strptime("18:00:00", "%H:%M:%S").time():
        ...
        print(f'Padrão 18hs: {soma_third_padrao}')
...
# Consulta SQL
query = f'''SELECT * FROM lw_octadesk.vw_report_diario; '''
...
destinatarios = ['C09KA4FPK2P'] #CANAL OFICIAL
...
for setor in setores:
    ...
    #if setor =='Suporte':
    #    soma_first_padrao, soma_second_padrao, soma_third_padrao = 418, 1191, 1991
    #else:
    #    soma_first_padrao, soma_second_padrao, soma_third_padrao = 92, 285, 462
    ...
    soma_first_semana_anterior_fone,soma_second_semana_anterior_fone,soma_third_semana_anterior_fone = soma_points(df, canal='Telefone',setor=setor,ultima_semana=True)
    ...
    soma_first_fone,soma_second_fone,soma_third_fone = soma_points(df, canal='Telefone',setor=setor,ultima_semana=0,data=True)
