...
config_folder = r'C:\Users\emerson.ramos\Desktop\projetos\KingHost\report diário'
...
def calcular_dias_uteis(data_inicio, data_fim):
    """
    Calcula a quantidade de dias úteis entre duas datas, excluindo fins de semana e feriados brasileiros.
    """
    feriados_brasil = holidays.Brazil()
    dias_uteis = 0
    ...
    # Iterar através de cada dia no período
...
def padrao(df, canal=None,setor= None, ultima_semana=None, data=None,diautil=None,padrao_inicio=None, padrao_fim=None):
    # Obter a hora atual
    hora_atual = datetime.now().time().replace(microsecond=0)
    ...
    # Calcular a quantidade REAL de dias úteis no período especificado
    if diautil and padrao_inicio and padrao_fim:
        qtd_datas_distintas = calcular_dias_uteis(padrao_inicio, padrao_fim)
        print(f'Dias úteis no período ({padrao_inicio.date()} a {padrao_fim.date()}): {qtd_datas_distintas}')
    else:
        # Contar a quantidade de datas distintas na coluna 'data'
        qtd_datas_distintas = df['data'].nunique()
        print(f'Dias distintos nos dados: {qtd_datas_distintas}')

    # Inicializar a soma
    total_first_padrao = df['first_point'].sum()  # sempre soma o primeiro ponto
    soma_first_padrao = round(total_first_padrao / qtd_datas_distintas) if qtd_datas_distintas > 0 else 0
    print(f"o primeiro padrao é : {soma_first_padrao}")
    ...
    if hora_atual >= datetime.strptime("14:00:00", "%H:%M:%S").time():
        ...
        soma_second_padrao = round(total_second_padrao / qtd_datas_distintas) if qtd_datas_distintas > 0 else 0
    # Verificar os intervalos de tempo e realizar a soma
    if hora_atual >= datetime.strptime("18:00:00", "%H:%M:%S").time():
        total_third_padrao = df['third_point'].sum()  # apenas soma se a condição for verdadeira
        soma_third_padrao = round(total_third_padrao / qtd_datas_distintas) if qtd_datas_distintas > 0 else 0
...
# Consulta SQL
query = f'''SELECT * FROM kinghost_octadesk.vw_report_diario_filtrada WHERE "Data Inicio" BETWEEN '2025-08-01 00:00:00' AND '{hoje_fim}'; '''
...
mensagem = {
    "blocks": [
        ...
        },
        {
            ...
            "text": {
                "type": "mrkdwn",
                "text": f" {saudacao} Estamos disponibilizando abaixo os dados do relatório *Report Diário* da Kinghost."
            }
        }
    ]    
}
...
#destinatarios = ['C07NSPQ69TL'] #Canal de teste
destinatarios = ['C08C34VKB5Y','U06V8A8GF5L'] #CANAL OFICIAL
...
for setor in setores:
    ...
    #Soma padrão
    soma_first_padrao, soma_second_padrao, soma_third_padrao = padrao(df,setor=setor,data=True,diautil=True,padrao_inicio=padrao_inicio,padrao_fim=padrao_fim)
    ...
    tma = calcular_tempo(df, coluna_tempo_atendimento,setor=setor, ignora_bot=True,data=True)

    # Calcula o tempo médio de espera, ignorando robôs
    tme = calcular_tempo(df, coluna_tempo_espera,setor=setor,data=True)
...
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    tabela = {
        "": ["Padrão Média dia Útil - Ago à Out 2025",
    ...
    plt.title(f"Report Diário {setor} Kinghost {data_formatada} | {hora_atual}", fontsize=14)
