...
try:
    ...
    subject_keyword = 'Integração Locaweb - Atendimentos KingHost (Chamados Interações)'
    ...
    # Ajuste para -3 horas
    adjusted_time = today - timedelta(hours=3)

    # Formatar a data e hora no formato desejado (exemplo: DD/MM/AAAA HH:MM:SS)
    data_hora_formatada = adjusted_time.strftime("%d/%m/%Y %H:%M:%S")
    ...
    if last_email and last_email.is_multipart():
        for part in last_email.get_payload():
            if part.get_filename():
                ...
                now = datetime.now().strftime('%Y%m%d_%H%M%S')  # Formato: AAAAMMDD_HHMMSS

                # Separar o nome do arquivo e sua extensão
                filename_base, file_extension = os.path.splitext(filename)

                # Adicionar data e hora ao nome do arquivo, antes da extensão
                filename_with_timestamp = f"{filename_base}_{now}{file_extension}"

                # Salva o anexo no diretório especificado com o nome atualizado
                with open(os.path.join(output_dir, filename_with_timestamp), 'wb') as f:
                    f.write(part.get_payload(decode=True))

                    mensagem = f" Arquivo {filename_with_timestamp} baixado com sucesso!"
                    print(mensagem)
